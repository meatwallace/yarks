...
test('Prepare from a shallow clone', async (t) => {
  ...
  t.deepEqual(
    (await gitCommitedFiles('HEAD', { cwd })).sort(),
    ['dist/file.js', 'package.json'].sort(),
  );
  const [commit] = await gitGetCommits(undefined, { cwd });
  t.is(
    commit.subject,
    `Release version ${nextRelease.version} from branch ${branch}`,
  );
...
test('Throw SemanticReleaseError if prepare config is invalid', (t) => {
  ...
  const options = {
    prepare: [
      ...
      { path: '@semantic-release/git', message, assets },
    ],
  };

  const errors = [
    ...t.throws(() =>
      t.context.m.verifyConditions({}, { options, logger: t.context.logger }),
    ),
  ];
