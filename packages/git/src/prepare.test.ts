...
test('Exclude CHANGELOG.md, package.json, package-lock.json, and npm-shrinkwrap.json if "assets" is defined without it', async (t) => {
  ...
  await outputFile(
    path.resolve(cwd, 'npm-shrinkwrap.json'),
    "{name: 'test-package'}",
  );
...
\${nextRelease.notes}`,
  ...
  t.is(
    commit.subject,
    `Release version ${nextRelease.version} from branch ${options.branch}`,
  );
  t.is(
    commit.body,
    `Last release: ${lastRelease.version}\n${nextRelease.notes}\n`,
...
test('Set the commit author and committer name/email based on environment variables', async (t) => {
  ...
  await prepare(
    {},
    { cwd, env, options, lastRelease, nextRelease, logger: t.context.logger },
  );
